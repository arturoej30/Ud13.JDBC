...
public class VentanaGestorTareas {
    ...
    // Declaracion. Variable que guardará el objeto encargado de gestionar todas las tareas
    // Se puede usar pq se ha importado modelo.GestorTareas
	...
	private void initialize() {
		...
		// Crea un gestor de tareas nuevo y lo guarda en la variable gestor.
    ...
    private void guardarCSV() {
        JFileChooser chooser = new JFileChooser();
     // Crea la ventana para que el usuario elija dónde guardar el archivo.
        
        chooser.setDialogTitle("Guardar tareas en CSV");
     // Cambia el título de la ventana.
        
        chooser.setFileFilter(new FileNameExtensionFilter("Archivos CSV (*.csv)", "csv"));
     // Solo muestra y permite trabajar con archivos CSV.
        
        chooser.setCurrentDirectory(new File("."));
     // Abre inicialmente la carpeta actual del proyecto.

        int resultado = chooser.showSaveDialog(frame); 
     // Muestra la ventana de guardar y espera a que el usuario pulse Guardar o Cancelar.

        if (resultado == JFileChooser.APPROVE_OPTION)
     // Comprueba si el usuario ha pulsado el botón Guardar
        
        {
            File archivo = chooser.getSelectedFile();
      // Obtiene el archivo y la ubicación elegidos por el usuario.

            if (!archivo.getName().toLowerCase().endsWith(".csv")) {
            	 // Comprueba si el nombre NO termina en ".csv".
            	
                archivo = new File(archivo.getAbsolutePath() + ".csv");
             // Añade automáticamente la extensión .csv al nombre.
            }

            try (BufferedWriter bw = Files.newBufferedWriter(archivo.toPath())) {
            	// Files.newBufferedWriter() necesita una ruta de tipo Path.
            	// archivo es un objeto File, por eso se convierte con toPath().
            	// Path es la forma moderna que usa Java para trabajar con archivos.
            	// Así se puede abrir el archivo y escribir texto dentro de él.
                // Abre el archivo para escribir texto dentro de él.
                ...
                for (Tarea t : gestor.obtenerTodasTareas())  {
                	// Obtiene todas las tareas del gestor.
                	// En cada vuelta del bucle, la variable t contiene una tarea.
                	// gestor es un objeto de la clase GestorTareas que administra todas las tareas.
                	// Se crea con gestor = new GestorTareas() al iniciar la ventana.

                             bw.write(t.getTitulo() + "," +
                    		 // Obtiene el título de la tarea y añade una coma.
                            		 
                            t.getDescripcion() + "," +
                             // Obtiene la descripción de la tarea y añade una coma.
                            
                            t.getFechaFormateada());
                            // Obtiene la fecha de la tarea y completa la línea del CSV.
            	...
            	// Se ejecuta si ocurre algún error al guardar el archivo.
                JOptionPane.showMessageDialog(frame,
                        "Error al guardar el archivo: " + e.getMessage(),
                        "Error",
                        JOptionPane.ERROR_MESSAGE);
             // Muestra una ventana con el mensaje de error.
            }
        }
    }
